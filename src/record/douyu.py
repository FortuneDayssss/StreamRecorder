...
class Douyu(RecorderBase):
    def js_encrypt(self, room_id, device_id, request_time):
        """
        generate post parameters v and sign by js engine
        todo: CryptoJS import && encrypt algorithm eval in different js engine
        :param room_id:
        :param device_id:
        :param request_time:
        :return: v, sign
        """
        encrypt_origin_js = requests.get("https://www.douyu.com/swf_api/homeH5Enc?rids={}".format(room_id)).content.decode()
        encrypt_origin_js = json.loads(encrypt_origin_js)["data"]["room{}".format(room_id)]
        encrypt_origin_js = encrypt_origin_js.replace("eval(strc)", "strc;")
        crypto_js = '''
        ...
        '''
        js_context = execjs.compile(crypto_js + ";" + encrypt_origin_js)
        encrypt_alg_js = js_context.eval('ub98484234(1,1,1)')
        ...
        encrypted_data = ctx.call('foo', room_id, device_id, request_time)
        logging.debug("js encrypt data: " + encrypted_data)
        return re.search("v=(\\d+)", encrypted_data).group(1), re.search("sign=(\\w+)", encrypted_data).group(1)

    def get_rtmp(self, room_id):
        device_id = uuid.uuid4().hex
        request_time = str(int(time.time()))
        version, sign = self.js_encrypt(room_id, device_id, request_time)
        post_data = {
            'cdn': 'tct-h5',
            'did': device_id,
            'iar': "1",
        ...
        if h5_play_data['error'] != 0:
            return None
        rtmp_url = h5_play_data['data']['rtmp_url'] + "/" + h5_play_data['data']['rtmp_live']
        logging.debug("rtmp live: " + rtmp_url)
        return rtmp_url
